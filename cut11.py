# -*- coding: utf-8 -*-
"""
Daniel Vega - 11. SANIYEDE DONDUR (yeni Veo YOK)
B-roll 11sn'de donar (temiz kitap karti), ses+altyazi bitene kadar o kare durur.
Girdi (diskte): broll.mp4 + voice.mp3 + public/image/186545.png
Cikti: full_ad.mp4
Calistir (C:\DanielVegaBooks icinden): python cut11.py
"""
...
def get_dur(path):
    ...
    if not m: return 18.0
    h, mi, s = m.groups(); return int(h) * 3600 + int(mi) * 60 + float(s)

N = get_dur("voice.mp3")
END = N + 0.4                       # ses bitene kadar
HOLD = max(0.0, END - FREEZE_AT)    # 11sn'den sonra donuk sure
print("ses: {:.1f}sn -> video: {:.1f}sn (11sn'de donar, {:.1f}sn donuk)".format(N, END, HOLD))
...
# 1) broll'u 11sn'de kes + dondur + tam ses
print("[1/2] 11sn'de donduruluyor + tam ses...")
subprocess.run([FF, "-y", "-i", "broll.mp4", "-i", "voice.mp3", "-filter_complex",
                "[0:v]trim=0:{:.2f},setpts=PTS-STARTPTS,tpad=stop_mode=clone:stop_duration={:.2f}[v];"
                "[1:a]apad=whole_dur={:.2f}[a]".format(FREEZE_AT, HOLD, END),
                "-map", "[v]", "-map", "[a]", "-t", "{:.2f}".format(END),
                "-c:v", "libx264", "-pix_fmt", "yuv420p", "-c:a", "aac", "voiced.mp4"],
               check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

# 2) altyazi + gercek kapak ortada (donuk karede durur)
print("[2/2] altyazi + kapak...")
...
try:
    ...
    print("\nBITTI -> full_ad.mp4  ({:.1f}sn, 11sn'de donar, tek kitap + tam ses)".format(END))
